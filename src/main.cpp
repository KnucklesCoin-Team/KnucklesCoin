...
#if defined(NDEBUG)
# error "Knucklescoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb85cdd82bc9f06e89c585d1aa596dcb6be4e7cfbcf0b35f8ee0c73c96aedb00c ");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Knucklescoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Knucklescoin Signed Message:\n";
...
{
    // Knucklescoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Knucklescoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Knucklescoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Knucklescoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Knucklescoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Knucklescoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9089ad37658134f8957d5ad7cf0f76155415bd074b90f77b06542c553cc6d52f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0446420934bb9648c7f76f8347bd23732542a87863ca84df5485eaadc4e6a414d553ef2d6366817ce480bc81832a577aebd6c9cddeab1988d20221414ca763a559") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515560887;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086778501;

        if (fTestNet)
        {
            block.nTime    = 1515560874;
            block.nNonce   = 386033249;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x300793f0d071fdea043d4a98b0b3baaa75d01a317d3a8d2410c16f041f4be0ce"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc5, 0xb2, 0xdc }; // Knucklescoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KnucklescoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KnucklescoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KnucklescoinMiner(CWallet *pwallet)
{
    printf("KnucklescoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("knucklescoin-miner");
    ...
    try { loop {
        ...
        printf("Running KnucklescoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KnucklescoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KnucklescoinMiner, pwallet));
}
